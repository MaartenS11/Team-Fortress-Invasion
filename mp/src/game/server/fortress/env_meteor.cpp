											    ...
											    Vector &vecTriggerMaxs )
{
	...
	if ( pEntity )
	{
		if ( pEntity->edict() )
		{
			VectorCopy( pEntity->CollisionProp()->CollisionSpaceMins(), vecTriggerMins );
			VectorCopy( pEntity->CollisionProp()->CollisionSpaceMaxs(), vecTriggerMaxs );
		}
	}
}
...
void CEnvMeteorSpawner::Precache( void )
{
	...
	{
		engine->PrecacheModel( strResourceMeteorModels[iType] );
	}
}
...
void CEnvMeteorSpawner::Activate( void )
{
	...
	// Force an update, so the shared data gets transfered!
	NetworkStateChanged();
...
//				NDebugOverlay::Box( vecImpactPoint, m_vecMin, m_vecMax, 0, 255, 0, 0, 5 );
				...
				for ( CEntitySphereQuery sphere( vecImpactPoint, m_Meteor.GetDamageRadius() ); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
				{
