...
void CBaseObject::SetBuilder( CBaseTFPlayer *pBuilder, bool moveobjects )
{
	TRACE_OBJECT( UTIL_VarArgs( "%0.2f CBaseObject::SetBuilder builder %s, moveobjects == %s\n", gpGlobals->curtime, 
		pBuilder ? STRING( pBuilder->pl.netname ) : "NULL", 
		...
		if ( !(m_fObjectFlags & OF_DONT_PREVENT_BUILD_NEAR_OBJ) )
		{
			...
			for ( int i = 0; i < iNumberOfNearbyEntities; i++ )
			{
				...
				if ( pEntity->IsSolid( ) )
				{
					...
					if ( pEntity == (CBaseEntity*)GetBuilder() )
						continue;
	...
	case POWERUP_BOOST:
		{
			...
			if ( GetHealth() < GetMaxHealth() )
			{
				...
				if ( pAttacker->IsPlayer() )
				{
					...
					Msg("(%.2f) %s repaired %s for %.2f health.\n", gpGlobals->curtime, STRING(pPlayer->pl.netname), GetClassname(), flAmount );
				}
				*/
...
void CBaseObject::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr )
{
	...
	// Just use the vecDir here for now ~hogsy
...
{	
	...
	do
	{
		...
		for( idx = g_DamageMap.First(); idx != g_DamageMap.InvalidIndex(); idx = g_DamageMap.Next(idx) )
		{
			...
			strcpy(szRowName, g_DamageMap.GetElementName(idx) );
			*strchr(szRowName,',') = '\0';

			char szRowNameComma[256];
			strcpy( szRowNameComma, szRowName );
			strcat( szRowNameComma, "," );
			...
			if( g_CompletedRows.Find(szRowName) == g_CompletedRows.InvalidIndex() )
			{
				...
				for( int ColumnIdx = g_UniqueColumns.First(); ColumnIdx != g_UniqueColumns.InvalidIndex(); ColumnIdx = g_UniqueColumns.Next( ColumnIdx ) )
				{
					...
					strcpy( szRowNameCommaColumn, szRowNameComma );					
					strcat( szRowNameCommaColumn, g_UniqueColumns.GetElementName( ColumnIdx ) );
...
{ 
	...
	// If I'm being built, only allow +use if I have a sapper on me
	if ( IsBuilding() && !HasSapper() )
		return 0;
...
void CBaseObject::ChangeBuilder( CBaseTFPlayer *pNewBuilder, bool moveobjects )
{
	...
	TRACE_OBJECT( UTIL_VarArgs( "%0.2f CBaseObject::ChangeBuilder old %s new %s, moveobjects\n", gpGlobals->curtime, 
		oldBuilder ? STRING( oldBuilder->pl.netname ) : "NULL",
		pNewBuilder ? STRING( pNewBuilder->pl.netname ) : "NULL",
		moveobjects ? "true" : "false" ) );
