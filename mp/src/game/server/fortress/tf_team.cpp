...
void CTFTeam::Precache( void )
{
	...
	enginesound->PrecacheSound( "vox/captured-beta-resource.wav" );
	enginesound->PrecacheSound( "vox/captured-charlie-resource.wav" );
	enginesound->PrecacheSound( "vox/resource-lost.wav" );
...
void CTFTeam::PrecacheTechnology( CBaseTechnology *pTech )
{
	...
	for (int i = 0; i < TFCLASS_CLASS_COUNT; i++ )
		if ( pTech->GetSoundFile(i) && (pTech->GetSoundFile(i)[0] != 0) )
			pTech->SetClassResultSound( i, enginesound->PrecacheSound( pTech->GetSoundFile(i) ) );
}
