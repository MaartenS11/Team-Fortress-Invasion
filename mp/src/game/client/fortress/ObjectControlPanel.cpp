	...
	: BaseClass( parent, panelName, g_hVGuiObjectScheme ) 
{
	...
	m_pEMPPanel = new CCommandChainingPanel( this, "EMPPanel" );
	m_pUnpoweredPanel = new CCommandChainingPanel( this, "UnpoweredPanel" );
	...
	m_pEMPPanel->SetZPos( -1 );
	m_pUnpoweredPanel->SetZPos( -1 );
...
bool CObjectControlPanel::Init( KeyValues* pKeyValues, VGuiScreenInitData_t* pInitData )
{
	...
	m_pEMPPanel->SetBounds( x, y, w, h );
	m_pUnpoweredPanel->SetBounds( x, y, w, h );
	...
	m_pEMPPanel->SetVisible( false );
	m_pUnpoweredPanel->SetVisible( false );
...
//-----------------------------------------------------------------------------
void CObjectControlPanel::OnTickUnpowered( C_BaseObject *pObj, C_BaseTFPlayer *pLocalPlayer )
{
	ShowDismantleButton( !(pObj->GetFlags() & OF_CANNOT_BE_DISMANTLED) && pObj->GetOwner() == pLocalPlayer );
}

void CObjectControlPanel::OnTickEMP( C_BaseObject *pObj, C_BaseTFPlayer *pLocalPlayer )
{
...
void CObjectControlPanel::OnTickDeteriorating( C_BaseObject *pObj, C_BaseTFPlayer *pLocalPlayer )
{
	...
	if ( pLocalPlayer && ClassCanBuild( pLocalPlayer->PlayerClass(), pObj->GetType() ) )
	{
		...
		Q_snprintf( buf, 256, "Buy for %d", nCost );
...
void CObjectControlPanel::OnTickDismantling( C_BaseObject *pObj, C_BaseTFPlayer *pLocalPlayer )
{
	...
	char buf[256];
	int nLen = Q_snprintf( buf, 256, "%d second", nSec );
	if (nSec != 1)
	{
		buf[nLen] = 's';
		++nLen;
...
vgui::Panel* CObjectControlPanel::TickCurrentPanel()
{
	...
	else if (!pObj->IsPowered())
	{
		m_pCurrentPanel = GetUnpoweredPanel();
...
void CObjectControlPanel::OnTick()
{
	...
	char buf[256];
	Q_snprintf( buf, 256, "Health: %d%%", (int)(pObj->HealthFraction() * 100.0f) );
	m_pHealthLabel->SetText( buf );

	C_BaseTFPlayer *pPlayer = pObj->GetOwner();
	if (pPlayer)
		Q_snprintf( buf, 256, "Owner: %s", pPlayer->GetPlayerName() );
	else
		Q_snprintf( buf, 256, "No Owner" );
