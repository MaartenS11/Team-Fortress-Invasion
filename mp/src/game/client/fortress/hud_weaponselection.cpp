...
//			*pszCurrentBinding - 
//-----------------------------------------------------------------------------
int	CHudWeaponSelection::KeyInput(int down, int keynum, const char *pszCurrentBinding)
{
	if (!down)
		return 1;
...
void CHudWeaponItemPanel::DrawBox(bool isbuildmenu, int x, int y, int wide, int tall, bool isactive, bool isvalid, float normalizedAlpha, int number)
{
	...
	Color	cTeamColor;
	if (team == TEAM_ALIENS)
		cTeamColor = Color(0, 0, 70, 20);
	else
		cTeamColor = Color(70, 0, 0, 20);

	Color numberColor = selection->m_TextColor;

	BaseClass::DrawBox(x, y, wide, tall, cTeamColor, normalizedAlpha);
	BaseClass::DrawBox(x+2, y+2, wide-4, tall-4, Color(0, 0, 0, 20), normalizedAlpha);

	if (isactive)
	{
		if (!isvalid)
		{
			BaseClass::DrawBoxFade(x, y, wide, tall, Color(30,0,0,10), normalizedAlpha, 255, 80, true);

			numberColor = selection->m_InvalidActiveColor;
		}
		else
		{
			BaseClass::DrawBoxFade(x, y, wide, tall, Color(50,50,50,10), normalizedAlpha, 255, 80, true);
