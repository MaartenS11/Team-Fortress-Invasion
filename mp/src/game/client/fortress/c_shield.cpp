...
void C_Shield::DrawShieldPoints(Vector* pt, Vector* normal, float* opacity)
{
	...
	if (mat_wireframe.GetInt() == 0)
		pRenderContext->Bind( m_pShield, (IClientRenderable*)this );
	else
		pRenderContext->Bind( m_pWireframe, (IClientRenderable*)this );
	IMesh* pMesh = pRenderContext->GetDynamicMesh( true, NULL, NULL );
...
void C_Shield::DrawShieldDecals( Vector* pt, bool hitDecals )
{
	...
	for ( int r = m_Decals.Size(); --r >= 0; )
	{
		...
		if (!hitDecals)
			pRenderContext->Bind( m_pPassDecal, (IClientRenderable*)this );
		else
			pRenderContext->Bind( passDecal ? m_pPassDecal2 : m_pHitDecal, (IClientRenderable*)this );
