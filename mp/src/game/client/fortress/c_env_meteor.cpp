...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void C_EnvMeteorSpawner::ReceiveMessage( const char *msgname, int length, void *data )
{
	BEGIN_READ( data, length );
	m_SpawnerShared.m_flStartTime = READ_LONG();
	m_SpawnerShared.m_flNextSpawnTime = READ_LONG();
	SetNextClientThink( m_SpawnerShared.m_flNextSpawnTime );
}
#endif
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool C_EnvMeteorTail::SimulateAndRender( Particle *pParticle, ParticleDraw* pParticleDraw, float &sortKey )
{
	...
	DrawFragment( pParticleDraw, vecEndPos, vecDelta, vecEndColor, vecStartColor,  
