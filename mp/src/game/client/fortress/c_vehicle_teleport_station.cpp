...
void CVehicleTeleportStationControlPanel::OnTick()
{
	...
	{
		Q_snprintf( buf, 256, "Driven by %s", pVehicle->GetDriverPlayer()->GetPlayerName() );
	...
	Q_snprintf( buf, 256, "Passengers %d/%d", nPassengerCount >= 1 ? nPassengerCount - 1 : 0, nMaxPassengerCount - 1 );
	...
	if ( pVehicle->GetOwner() == C_BaseTFPlayer::GetLocalPlayer() )
	{
		if (nPassengerCount == nMaxPassengerCount)
		{
			...
			Q_snprintf( buf, 256, "Get In (Ejecting %s)", pPlayer->GetPlayerName() );
			m_pDriverLabel->SetText( buf );
