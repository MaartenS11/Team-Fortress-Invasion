...
void C_BaseObject::DrawRunningEffects( void )
{
	...
	if ( GetAttachment( iSmokeAttachment, vecSmoke, angSmoke ) )
	{
		...
		ObjectSmokeParticle *pParticle = (ObjectSmokeParticle *) pSmokeEmitter->AddParticle( sizeof(ObjectSmokeParticle), hSphereMaterial, vecSmoke );
		if ( pParticle )
		{
...
void C_BaseObject::DrawDamageEffects( void )
{
	...
	pSmokeEmitter->SetSortOrigin( vecSmoke );
	PMaterialHandle	hSphereMaterial = pSmokeEmitter->GetPMaterial( "particle/particle_noisesphere" );
	SimpleParticle *pParticle = (SimpleParticle *) pSmokeEmitter->AddParticle( sizeof(SimpleParticle), hSphereMaterial, vecSmoke );
	if ( pParticle )
	{
		pParticle->m_flLifetime	= 0.0f;
		pParticle->m_flDieTime	= random->RandomFloat( 2.0f, 3.0f );
