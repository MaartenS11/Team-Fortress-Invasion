...
{
	strcpy( m_szTextureName, texture );
}
...
void CBaseTechnology::SetClassResultSound( int iClass, const char *pSound )
{
	...
	char sSoundFile[256];
	Q_snprintf( sSoundFile,sizeof(sSoundFile), "vox/tech/%s.wav", pSound );

	// Class of 0 is the team's sound file
	if ( iClass == 0 )
	{
		Q_strncpy( m_pszTeamSoundFile, sSoundFile, sizeof(m_pszTeamSoundFile) );
	}
	else
	{
		m_ClassResults[iClass].bClassTouched = true;
		Q_strncpy( m_ClassResults[iClass].pszSoundFile, sSoundFile, sizeof(m_ClassResults[iClass].pszSoundFile) );
	}
}
