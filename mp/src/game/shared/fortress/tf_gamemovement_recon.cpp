	...
	if ( tr.DidHitWorld() )
	{
		...
		if ( pTFMove )
		{
				 ...
				 ( pTFMove->ReconData().m_flSuppressionImpactTime == TIME_WALL_INVALID ) )
			{
				...
				MoveHelper()->StartSound( pTFMove->GetAbsOrigin(), "Recon.WallJump" );
...
//-----------------------------------------------------------------------------
void CTFGameMovementRecon::CheckWaterJump()
{
}
...
bool CTFGameMovementRecon::CheckWallJump( CTFMoveData *pTFMove )
{
	...
	// Play a jump sound
	player->PlayStepSound((Vector&)mv->GetAbsOrigin(),player->m_pSurfaceData,1.0f,true);
...
// Purpose:
//-----------------------------------------------------------------------------
Vector CTFGameMovementRecon::GetPlayerMins( bool bDucked ) const
{
...
// Purpose:
//-----------------------------------------------------------------------------
Vector CTFGameMovementRecon::GetPlayerMaxs( bool bDucked ) const
{
...
// Purpose:
//-----------------------------------------------------------------------------
Vector CTFGameMovementRecon::GetPlayerViewOffset( bool bDucked ) const
{
