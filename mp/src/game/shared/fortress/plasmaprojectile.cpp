...
void CBasePlasmaProjectile::MissileTouch( CBaseEntity *pOther )
{
	...
	if ( m_flDamage && !m_flExplosiveRadius )
	{
		...
		// Updated, as "MediumRound" doesn't exist in Invasion ~hogsy
		CalculateBulletDamageForce( &info, GetAmmoDef()->Index(/*"MediumRound"*/"Bullets"), GetAbsVelocity(), vecDamageOrigin );
